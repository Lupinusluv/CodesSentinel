...
def test_dedup_collapses_cross_category_adjacent_synonymous():
    ...
    issues = [
        _issue("security", "critical", line_start=41, file_path="a.py",
               description="SQL injection vulnerability"),
        _issue("style",    "warning",  line_start=42, file_path="a.py",
...
def test_same_category_far_lines_keeps_both():
    ...
    issues = [
        _issue("security", "critical", line_start=10, file_path="a.py",
               description="hardcoded credential"),
        _issue("security", "warning",  line_start=20, file_path="a.py",
               description="hardcoded credential"),
    ]
    result = deduplicate_issues(issues)
    assert len(result) == 2
...
def test_cross_category_same_line_dissimilar_keeps_both():
    ...
    issues = [
        _issue("security",    "critical", line_start=42, file_path="a.py",
               description="SQL injection via string formatting"),
        _issue("performance", "warning",  line_start=42, file_path="a.py",
...
def test_whole_file_none_lines_dissimilar_keeps_both():
    ...
    issues = [
        _issue("security", "critical", line_start=None, file_path="a.py",
               description="missing input validation across module"),
        _issue("style",    "warning",  line_start=None, file_path="a.py",
...
def test_output_sorted_by_severity():
    ...
    issues = [
        _issue("style",       "suggestion", line_start=1,
               description="missing type hints on function"),
        _issue("security",    "critical",   line_start=2,
               description="path traversal in file open"),
        _issue("performance", "warning",    line_start=3,
               description="redundant database round trip"),
    ]
    result = deduplicate_issues(issues)
